...
class deplacement_invalide(Exception):
    pass
...
class Echiquier:
    ...
    def deplacer(self, position_source, position_cible):
        ...
        if position_source == position_cible:
            raise reclic_sur_piece()
            return False
        elif not self.deplacement_est_valide(position_source, position_cible):
            raise deplacement_invalide()
            return False

        #Sauvegarder le deplacement
        piece_cible = self.recuperer_piece_a_position(position_cible)
        piece_source = self.recuperer_piece_a_position(position_source)
        self.deplacements.append([piece_source.couleur, position_source, position_cible, piece_cible])
